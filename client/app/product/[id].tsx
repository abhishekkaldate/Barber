...
export default function ProductDetails() {
    ...
    const handleAddToCart = ()=> {
        if(!selectedSize){
            Toast.show({
                type: 'info',
                text1: 'Please select a Fade',
                text2: 'Select a Fade to add this service to your basket',
            })
            return;
        }
    ...
    return (
                ...
                <View className='relative h-[450px] bg-gray-100 mb-6'>
                    ...
                    <View className='absolute bottom-4 left-0 right-0 flex-row justify-center gap-2'>
                        {product.images?.map((_, index) => (
                            <View key={index} className={`h-2 rounded-full ${index === activeImageIndex ? 'bg-primary' : 'w-2 bg-gray-300'}`} />
                        ))}
                    </View>
        ...
        <View className='px-5'>
                        ...
                        {product.sizes && product.sizes.length > 0 &&(
                            ...
                            <View className='flex-row gap-3 mb-6 flex-wrap'>
                                {product.sizes.map((size) => (
                                    ...
                                    ${selectedSize === size ? 'bg-primary border-primary' : 'bg-white border-gray-300'}`}>
                                        <Text className={`text-sm font-medium ${selectedSize === size ? 'text-white' : 'text-primary'}`}>Fade</Text>
                        ...
                        <View className='absolute bottom-0 left-0 flex-row right-0 p-4 bg-white border-t border-gray-100'>
                            <TouchableOpacity onPress={handleAddToCart} className='w-4/5 bg-primary py-4 rounded-full items-center shadow flex-row justify-center'>
                                <Ionicons name='bag-outline' size={20} color="white"/>
                                <Text className='text-white font-bold text-base ml-2'>Add to Basket</Text>
                            </TouchableOpacity>
